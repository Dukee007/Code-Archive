import discord, time, asyncio, os, random, json, youtube_dl, functools, itertools, math, ffmpeg
...
class Music(commands.Cog):
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester or voter.id == 508372340904558603:
            await ctx.message.add_reaction('⏭')
    ...
    async def _play(self, ctx: commands.Context, *, search: str = None):
        ...
        """
        try:
            if ctx.voice_client.is_paused() == True and search == None:
                try:
